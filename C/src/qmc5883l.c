...
static float A[3][3] = {
    {0.973931f, 0.009692f, -0.008819f},
    {0.009692f, 1.118619f, 0.031196f},
    {-0.008819f, 0.031196f, 0.965646f},
};
...
{
    if (write(qmc->fd, &reg, 1) != 1)
        return -1;
    if (read(qmc->fd, data, len) != len)
        return -1;
    return 0;
}
...
int QMC5883L_Init(QMC5883L_Type *qmc, const char *i2c_device, uint8_t address, uint8_t mode, uint8_t data_output_rate, uint8_t full_scale, uint8_t over_sample)
{
    ...
    uint8_t data = (over_sample << 6) | (full_scale << 4) | (data_output_rate << 2) | mode;
...
void QMC5883L_Read(QMC5883L_Type *qmc)
{
    ...
    qmc->field_mag = sqrtf(qmc->field[0] * qmc->field[0] + qmc->field[1] * qmc->field[1] + qmc->field[2] * qmc->field[2]);
}
...
int QMC5883L_Calibrate(QMC5883L_Type *qmc, const char *filename, uint32_t samples, uint32_t period_us)
{
    ...
    for (uint32_t i = 0; i < samples; i++)
    {
        ...
        qmc->field[0] = (float)qmc->field_raw_16[0]/ qmc->GAIN_LSB ;
        qmc->field[1] = (float)qmc->field_raw_16[1]/ qmc->GAIN_LSB ;
        qmc->field[2] = (float)qmc->field_raw_16[2]/ qmc->GAIN_LSB ;
